...
macro(setup_cuvslam_settings)
    ...
    if(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
        ...
        if (APPLE AND ${CMAKE_HOST_SYSTEM_PROCESSOR} MATCHES "arm64|aarch64")
            # The macOS build is redistributed as a prebuilt archive, so it targets the oldest
            # Apple silicon rather than the build machine: -march=native on an M4/M5 emits
            # instructions an M1 traps on.
            target_compile_options(cuvslam_settings INTERFACE -mcpu=apple-m1)
        ...
        if(APPLE)
            # ld64 has no --no-undefined/--no-allow-shlib-undefined and treats -s as obsolete.
            target_link_options(cuvslam_settings INTERFACE LINKER:-undefined,error)
        else()
            target_link_options(cuvslam_settings INTERFACE
...
function(setup_test_project)
    ...
    if(USE_CUMETAL)
        # cuVSLAM's image kernels read through texture objects, whose handles are dereferenced on the
        # GPU. That only works when cudaMalloc hands out real Metal device addresses; under CuMetal's
        # default CPU-shared mapping the kernels silently read zeros. libcuvslam is always run this
        # way, so the tests must declare the same mode instead of inheriting whatever the shell has.
