...
// libc++ marks the floating-point std::from_chars unavailable before macOS 26, so float goes
// through strtof. from_chars' stricter grammar is re-imposed: no leading blank, no leading '+',
// and the whole string has to be consumed.
template <>
float Parse<float>(std::string_view v, std::string_view expected) {
