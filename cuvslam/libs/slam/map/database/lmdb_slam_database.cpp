...
bool LmdbSlamDatabase::WriteTxn::Put(MDB_dbi dbi, const Key& id, const Blob& blob) {
  MDB_val key;
  MDB_val data;
...
bool LmdbSlamDatabase::WriteTxn::Del(MDB_dbi dbi, const Key& id) {
  MDB_val key;
  key.mv_size = sizeof(id);
  key.mv_data = (void*)&id;
...
bool LmdbSlamDatabase::ReadTxn::Get(MDB_dbi dbi, const Key& id, Blob& blob) {
  MDB_val key;
  key.mv_size = sizeof(id);
  key.mv_data = (void*)&id;
...
bool LmdbSlamDatabase::ReadTxn::Get(MDB_dbi dbi, const Key& id, MDB_val& data) {
  MDB_val key;
  key.mv_size = sizeof(id);
  key.mv_data = (void*)&id;
...
bool LmdbSlamDatabase::Cursor::Get(Key& id, Blob& blob) {
  MDB_val key;
  MDB_val data;
  if (!Get(&key, &data)) {
    return false;
...
bool LmdbSlamDatabase::Cursor::Get(Key& id, MDB_val& data) {
  MDB_val key;
  if (!Get(&key, &data)) {
    return false;
  }
