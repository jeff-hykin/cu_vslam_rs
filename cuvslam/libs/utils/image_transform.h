...
struct _ImageTransform {
  ...
  static ImageMatrix<_PixelType> LinearToLog(const ImageMatrix<_PixelType>& image) {
    ...
    // std::ptr_fun was removed in C++17; libc++ does not provide it at all.
    const auto& logArray =
        linArray.unaryExpr([](const _PixelType value) { return LINtoSRGB(value); }).array() * _PixelType(255.0f);
    return logArray;
#else
